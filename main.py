...
def train_and_test(config, data_loader_dict):
    ...
    elif config.MODEL.NAME == 'LQC2F':
        model_trainer = trainer.LQC2FTrainer.LQC2FTrainer(config, data_loader_dict)
    else:
...
if __name__ == "__main__":
    ...
    if config.TOOLBOX_MODE == "train_and_test":
        ...
        if (config.TRAIN.DATA.DATASET and config.TRAIN.DATA.DATA_PATH):
            ...
            if config.TRAIN.DATA.DATASET == "V4V":
        ...
        if (config.VALID.DATA.DATASET and config.VALID.DATA.DATA_PATH and not config.TEST.USE_LAST_EPOCH):
            if config.VALID.DATA.DATASET == "V4V":
                valid_data = valid_loader(
                    name="valid",
                    data_path=config.VALID.DATA.DATA_PATH,
                    config_data=config.VALID.DATA,
                    split="val")  # V4V uses "val" folder for validation
            else:
                valid_data = valid_loader(
                    name="valid",
                    data_path=config.VALID.DATA.DATA_PATH,
    ...
    if config.TOOLBOX_MODE == "train_and_test" or config.TOOLBOX_MODE == "only_test":
        ...
        if config.TEST.DATA.DATASET and config.TEST.DATA.DATA_PATH:
            if config.TEST.DATA.DATASET == "V4V":
