...
def calculate_metrics_return(predictions, labels, config):
    """Calculate rPPG Metrics (MAE, RMSE, MAPE, Pearson Coef.).

    Returns:
        dict: Contains at least {'MAE': float}. Additional fields may be included.
    """
    ...
    if config.INFERENCE.EVALUATION_METHOD == "FFT":
        ...
        if num_test_samples > 0:
            # Always compute MAE for downstream selection, regardless of TEST.METRICS
            results["MAE"] = float(np.mean(np.abs(predict_hr_fft_all - gt_hr_fft_all)))
            # Optionally include other summary metrics
            results["RMSE"] = float(np.sqrt(np.mean(np.square(predict_hr_fft_all - gt_hr_fft_all))))
            with np.errstate(divide='ignore', invalid='ignore'):
                mape_vals = np.abs((predict_hr_fft_all - gt_hr_fft_all) / gt_hr_fft_all) * 100
                mape_vals = mape_vals[~np.isnan(mape_vals)] if mape_vals.size else mape_vals
            results["MAPE"] = float(np.mean(mape_vals)) if mape_vals.size else float('inf')
            if predict_hr_fft_all.size >= 2:
                pearson = np.corrcoef(predict_hr_fft_all, gt_hr_fft_all)[0][1]
                results["Pearson"] = float(pearson)
            results["SNR"] = float(np.mean(SNR_all)) if SNR_all.size else float('nan')
        ...
        if num_test_samples > 0:
            # Always compute MAE for downstream selection, regardless of TEST.METRICS
            results["MAE"] = float(np.mean(np.abs(predict_hr_peak_all - gt_hr_peak_all)))
            results["RMSE"] = float(np.sqrt(np.mean(np.square(predict_hr_peak_all - gt_hr_peak_all))))
            with np.errstate(divide='ignore', invalid='ignore'):
                mape_vals = np.abs((predict_hr_peak_all - gt_hr_peak_all) / gt_hr_peak_all) * 100
                mape_vals = mape_vals[~np.isnan(mape_vals)] if mape_vals.size else mape_vals
            results["MAPE"] = float(np.mean(mape_vals)) if mape_vals.size else float('inf')
            if predict_hr_peak_all.size >= 2:
                pearson = np.corrcoef(predict_hr_peak_all, gt_hr_peak_all)[0][1]
                results["Pearson"] = float(pearson)
            results["SNR"] = float(np.mean(SNR_all)) if SNR_all.size else float('nan')
        for metric in config.TEST.METRICS:
            if metric == "MAE":
