...
class EfficientPhys(nn.Module):
    ...
    def __init__(self, in_channels=3, nb_filters1=32, nb_filters2=64, kernel_size=3, dropout_rate1=0.25,
                 dropout_rate2=0.5, pool_size=(2, 2), nb_dense=128, frame_depth=20, img_size=36, target_size=36, channel='raw'):
        """Definition of EfficientPhys.
        Args:
          in_channels: the number of input channel. Default: 3
          img_size: original frame size (e.g. 128). Default: 36.
          target_size: size to resize input (e.g. 96). Default: 36.
          channel: input type. Default: raw
        Returns:
          EfficientPhys model.
        """
        ...
        # Dense layers - target_size 기준으로 설정 (입력이 target_size로 리사이즈되므로)
        if target_size == 36:
            self.final_dense_1 = nn.Linear(3136, self.nb_dense, bias=True)  # (36/2/2)^2 * 64 = 9*9*64
        elif target_size == 72:
            self.final_dense_1 = nn.Linear(16384, self.nb_dense, bias=True)  # (72/2/2)^2 * 64 = 18*18*64
        elif target_size == 96:
            self.final_dense_1 = nn.Linear(30976, self.nb_dense, bias=True)  # (96/2/2)^2 * 64 = 24*24*64
        elif target_size == 128:
            self.final_dense_1 = nn.Linear(57600, self.nb_dense, bias=True)  # (128/2/2)^2 * 64 = 32*32*64
        else:
    ...
    def forward(self, inputs, params=None):
        # # 입력 이미지를 target_size로 다운샘플링
        # b, c, h, w = inputs.shape
        # if h != self.target_size or w != self.target_size:
        #     inputs = F.interpolate(inputs, size=(self.target_size, self.target_size), mode='area')
